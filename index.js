...
function embaralhaPerguntas(questoes) {
  return questoes.sort(() => Math.random() - 0.5);
}
function selecionarPerguntasAleatorias(questoes, quantidade) {
  const perguntasEmbaralhadas = embaralhaPerguntas(questoes);
  return perguntasEmbaralhadas.slice(0, quantidade);
}

function exibirPerguntas(questoes) {
  questoes.forEach((pergunta, index) => {
    console.log(`pergunta ${index + 1}: ${pergunta.pergunta}`);
    let respostaUsuario = input.question("digite sua resposta: ");
    validaResposta(pergunta.resposta, respostaUsuario);
  })
}

function validaResposta (respostaUsuario, resposta) {
...
function mensagemFinal(pontos) {
  if(pontos == 0) {
    console.log("voce nao acertou nenhum ponto que pena!");
  } else if (pontos > 0 && pontos < 5){
    console.log("voce acertou menos da metade");
  } else if (pontos >= 5 && pontos < 9){
    console.log("Voce acertou mais da metade!!");
  } else {
    console.log("voce acertou todas parabens!!!")
  }
}
